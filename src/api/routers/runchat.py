...
router = APIRouter(prefix="/educhat", tags=["Educhat"])
...
async def run_educhat_api(body : RundEduChat):
    ...
    agent_response = run_educhat(user_id=body.user_id, chat_id=body.chat_id, human_message=body.human_message, system_prompt=body.system_prompt, educhat=educhat)
